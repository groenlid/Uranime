...
<script type="text/javascript">
	...
	function getInitialNewsFeed(){
		$.getJSON('/api/newsfeed/10.json', function(data) {
		  var items = [];
	...
	function getNewsFeed(){
		$.getJSON('/api/newsfeedAfter/'+last+'.json', function(data) {
		  var items = [];
	...
	function activityItem(item){
		...
		{
			case "fanart":
				animeData = getAnimeInfo(item.Activity.object_id);
				statusText = ' fanart on anime <strong><a href="/anime/view/'+item.Activity.object_id+'">' + animeData.anime.Anime.title + '</a></strong>';
			break;
			case "image":
				animeData = getAnimeInfo(item.Activity.object_id);
				statusText = ' image on anime <strong><a href="/anime/view/'+item.Activity.object_id+'">' + animeData.anime.Anime.title + '</a></strong>';
			break;
			case "episode":
				episodeData = getEpisodeInfo(item.Activity.object_id);
				animeData = getAnimeInfo(episodeData.episode.Episode.anime_id);
				statusText = ' <strong><a href="/episode/view/' + episodeData.episode.Episode.id + '">episode ' + episodeData.episode.Episode.number + '</a></strong> of anime <strong><a href="/anime/view/'+animeData.anime.Anime.id+'">' + animeData.anime.Anime.title + '</a></strong>';
			break;
			case "reference":
				animeData = getAnimeInfo(item.Activity.object_id);
				statusText = ' a reference for '+item.Activity.option+' for anime  <strong><a href="/anime/view/'+item.Activity.object_id+'">' + animeData.anime.Anime.title + '</a></strong>';
			break;
			case "anime":
				animeData = getAnimeInfo(item.Activity.object_id);
				statusText = ' a spear through the whale\'s blowhole and added a new anime <strong><a href="/anime/view/'+item.Activity.object_id+'">' + animeData.anime.Anime.title + '</a></strong>';
			break;
		}
	...
	function getAnimeInfo(id){
		...
		{
			var animeData = synchronousAJAX("/api/viewAnime/" + id +".json");
	...
	function getEpisodeInfo(id){
		...
		if(retrievedObject == undefined)
		{
			var episodeData = synchronousAJAX("/api/viewEpisode/" + id +".json");
